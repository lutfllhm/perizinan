// Service untuk operasi data karyawan
const bcrypt = require('bcryptjs');

/**
 * Data karyawan lengkap dari 8 kantor (173 karyawan)
 */
const dataKaryawanLengkap = {
  'RBM-IWARE SURABAYA': [
...
/**
 * Import otomatis data karyawan ke database
 */
async function importOtomatisKaryawan(db) {
  try {
    // Cek apakah tabel karyawan sudah ada data
    const [existing] = await db.query('SELECT COUNT(*) as jumlah FROM karyawan');
    if (existing[0].jumlah > 0) {
      console.log(`ℹ️  Data karyawan sudah ada (${existing[0].jumlah} record), skip import`);
      return;
    }
    
    console.log('🔄 Memulai import otomatis data karyawan...');
    
    let totalDiimport = 0;
    let counterTelepon = 1000;
    
    for (const [kantor, daftarKaryawan] of Object.entries(dataKaryawanLengkap)) {
      for (const karyawan of daftarKaryawan) {
        const noTelp = `08123456${String(counterTelepon).padStart(4, '0')}`;
        counterTelepon++;
        
        try {
          await db.query(
            'INSERT INTO karyawan (kantor, nama, jabatan, departemen, no_telp, jatah_cuti, sisa_cuti, status) VALUES (?, ?, ?, ?, ?, 12, 12, "aktif")',
            [kantor, karyawan.nama, karyawan.jabatan, karyawan.departemen, noTelp]
          );
          totalDiimport++;
        } catch (error) {
    ...
    console.log(`✅ Berhasil import ${totalDiimport} karyawan dari 8 kantor`);
    
  } catch (error) {
    console.error('❌ Error import otomatis karyawan:', error.message);
  }
}

module.exports = {
  dataKaryawanLengkap,
  importOtomatisKaryawan
};
