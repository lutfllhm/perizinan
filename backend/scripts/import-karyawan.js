...
// Data karyawan dari PengajuanForm.jsx
const dataKaryawan = {
  'RBM-IWARE SURABAYA': [
...
async function importKaryawan() {
  ...
  try {
    console.log('🔄 Memulai import data karyawan...');
      ...
      for (const karyawan of karyawanList) {
        try {
          await connection.query(
            `INSERT INTO karyawan (kantor, nama, jabatan, departemen, jatah_cuti, sisa_cuti, tahun_cuti) 
             VALUES (?, ?, ?, ?, 12, 12, YEAR(CURDATE()))`,
            [kantor, karyawan.nama, karyawan.jabatan, karyawan.departemen]
          );
          totalImported++;
        } catch (error) {
