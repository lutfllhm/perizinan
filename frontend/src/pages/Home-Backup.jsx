      ...
      <section className="relative z-10 min-h-screen flex items-center justify-center px-4 sm:px-6 pt-16">
        <div className="relative max-w-5xl mx-auto text-center">
          <div className={`transition-all duration-1000 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
            ...
            >
              ...
              >
                <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-purple-400">
                  <TypingAnimation text="Cuti & Lembur" speed={80} />
                </span>
              </motion.div>
            </h1>
            ...
            >
              {[
                { icon: '⚡', title: 'Cepat', desc: 'Proses Instan', delay: 0 },
                { icon: '📱', title: 'Mudah', desc: 'User Friendly', delay: 0.1 },
                { icon: '🔔', title: 'Real-Time', desc: 'Notifikasi Langsung', delay: 0.2 }
              ].map((stat, index) => (
                <motion.div
                  key={index}
                  ...
                  className={`col-span-${index === 2 ? '2 md:col-span-1' : '1'}`}
                >
                  {isMobile ? (
                    <div className="group bg-white/10 backdrop-blur-sm border border-white/20 rounded-xl sm:rounded-2xl p-4 sm:p-6 transition-all duration-300 cursor-pointer active:scale-95">
                      <div className="text-xl sm:text-2xl mb-2">{stat.icon}</div>
                      <div className="text-lg sm:text-2xl font-bold text-white mb-1">{stat.title}</div>
                      <div className="text-xs sm:text-sm text-slate-300">{stat.desc}</div>
                    </div>
                  ) : (
                    <Card3D className="group bg-white/10 backdrop-blur-sm border border-white/20 rounded-xl sm:rounded-2xl p-4 sm:p-6 transition-all duration-300 cursor-pointer hover:bg-white/15 hover:border-white/30">
                      <div className="text-xl sm:text-2xl mb-2 transition-transform duration-300 group-hover:scale-125 group-hover:rotate-12">{stat.icon}</div>
                      <div className="text-lg sm:text-2xl font-bold text-white mb-1">{stat.title}</div>
                      <div className="text-xs sm:text-sm text-slate-300">{stat.desc}</div>
                    </Card3D>
                  )}
      ...
      <section ref={featuresRef} className="relative z-10 py-16 sm:py-20 md:py-24 px-4 sm:px-6 bg-black/20 backdrop-blur-sm">
        <div className="max-w-5xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 sm:gap-8 px-4 sm:px-0">
            {[
              { icon: '📝', title: 'Pengajuan Cuti', desc: 'Ajukan cuti tahunan, sakit, atau izin khusus dengan mudah. Sistem akan otomatis menghitung sisa kuota cuti Anda.', gradient: 'from-blue-500 to-cyan-500', delay: 0 },
              { icon: '⏰', title: 'Pengajuan Lembur', desc: 'Catat jam lembur Anda dengan akurat. Sistem akan menghitung kompensasi sesuai dengan kebijakan perusahaan.', gradient: 'from-purple-500 to-pink-500', delay: 0.1 },
              { icon: '✅', title: 'Tracking Status', desc: 'Pantau status pengajuan Anda secara real-time. Dapatkan notifikasi instant saat ada update.', gradient: 'from-green-500 to-emerald-500', delay: 0.2 },
              { icon: '📊', title: 'Riwayat Lengkap', desc: 'Akses riwayat pengajuan Anda kapan saja. Semua data tersimpan dengan aman dan terorganisir.', gradient: 'from-orange-500 to-red-500', delay: 0.3 }
            ].map((feature, index) => (
              <motion.div
                key={index}
              ...
              >
                {isMobile ? (
                  <div className="group bg-white/10 backdrop-blur-sm border border-white/20 rounded-3xl p-6 sm:p-8 transition-all duration-300 cursor-pointer active:scale-95">
                    <div className={`w-14 h-14 sm:w-16 sm:h-16 bg-gradient-to-r ${feature.gradient} rounded-2xl flex items-center justify-center text-xl sm:text-2xl mb-4 sm:mb-6 shadow-lg`}>
                  ...
                  <Card3D className="group bg-white/10 backdrop-blur-sm border border-white/20 rounded-3xl p-6 sm:p-8 transition-all duration-300 cursor-pointer hover:bg-white/15 hover:border-white/30">
                    <div className={`w-14 h-14 sm:w-16 sm:h-16 bg-gradient-to-r ${feature.gradient} rounded-2xl flex items-center justify-center text-xl sm:text-2xl mb-4 sm:mb-6 transition-all duration-300 shadow-lg group-hover:scale-110 group-hover:rotate-6 group-hover:shadow-2xl`}>
                      <span className="transition-transform duration-300 group-hover:scale-125">{feature.icon}</span>
                    </div>
                    <h3 className="text-xl sm:text-2xl font-bold text-white mb-3 sm:mb-4 transition-all duration-300 group-hover:text-transparent group-hover:bg-clip-text group-hover:bg-gradient-to-r group-hover:from-blue-400 group-hover:to-purple-400">
                      {feature.title}
                    </h3>
                    <p className="text-sm sm:text-base text-slate-200 leading-relaxed transition-colors duration-300 group-hover:text-white">
                      {feature.desc}
                    </p>
                  </Card3D>
                )}
